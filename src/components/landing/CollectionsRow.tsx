...
import { CollectionShareButton } from '@/components/share/CollectionShareButton';
      ...
      <div className="overflow-x-auto scrollbar-hide -mx-3 px-3">
        <div className="flex gap-2 pb-2 w-max">
          {collections.map((collection) => (
            <div key={collection.id} className="relative flex-shrink-0">
              <button
                onClick={() => handleClick(collection)}
                className={cn(
                  "w-16 rounded-lg overflow-hidden transition-all duration-200",
              ...
              {/* Share button - appears on hover/active */}
              <div className={cn(
                "absolute -top-1 -right-1 opacity-0 transition-opacity",
                activeCollectionId === collection.id && "opacity-100"
              )}>
